      ...
      <Link href={`/blog/${post.slug}`} className="group block">
        ...
        >
          ...
          <div className="p-5">
            ...
            <div className="flex items-center justify-center gap-6 py-3 bg-[#1a1510] rounded border border-amber-900/20">
              {/* Left spool */}
              <motion.div
                animate={{ rotate: 360 }}
                transition={{ duration: 3, repeat: Infinity, ease: "linear" }}
                className="w-8 h-8 rounded-full border-2 border-amber-800/30 flex items-center justify-center"
              >
                <div className="w-3 h-3 rounded-full bg-amber-900/40" />
              </motion.div>

              {/* Tape */}
              <div className="flex-1 h-0.5 bg-amber-900/40 rounded" />
